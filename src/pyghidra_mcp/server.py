...
from fastmcp import FastMCP, Context  # <-- UPDATED: import from fastmcp
...
def make_mcp_object() -> FastMCP:
    """
    Creates and returns an instance of FastMCP with all tools registered,
    with the server lifespan context manager attached.
    """
    mcp = FastMCP("pyghidra-mcp", lifespan=server_lifespan)  # UPDATED: now fastmcp.FastMCP

    # MCP Tools (identical registration -- unchanged)
    @mcp.tool()
